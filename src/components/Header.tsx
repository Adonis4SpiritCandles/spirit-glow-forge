      ...
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="hidden md:flex h-16 items-center justify-between">
          ...
          <div className="flex items-center gap-1 md:gap-1 lg:gap-3">
            ...
            {/* Admin Button - Always visible on tablet/desktop if admin */}
            {user && userProfile?.role === 'admin' && (
              <Link to="/admin">
                <Button variant="ghost" size="sm" className="relative">
                  <img src={goldShieldIcon} alt="Admin" className="h-3.5 w-3.5 md:h-4 md:w-4" />
                  {unseenCount > 0 && (
                    <Badge
                      variant="secondary" 
                      className="absolute -top-2 -right-2 h-5 w-5 flex items-center justify-center p-0 bg-red-500 text-white text-xs animate-pulse"
            ...
            {/* User Dropdown Menu - Tablet/Desktop */}
            {user ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
